...
struct Tileset {
    filename: String,
    used_tiles: HashMap<u8, HashSet<usize>>,
}

impl Tileset {
    fn new(filename: String, used_tiles: HashMap<u8, HashSet<usize>>) -> Self {
        Self {
            filename,
            used_tiles,
        }
    ...
    fn remove_uninteresting_tiles(&mut self, tiles_of_interest: &HashSet<u8>) {
        self.used_tiles = self.used_tiles.iter()
            .filter(|&(k, _)| tiles_of_interest.contains(k))
            .map(|(k, v)| (k.clone(), v.clone()))
            .collect();
    }

    fn used_tiles_len(&self) -> usize {
        self.used_tiles.len()
    }
}
...
impl Map {
    fn new(name: String, tileset: String) -> Self {
...
fn get_used_tiles_per_tileset() -> Result<(Vec<Tileset>, HashSet<String>), Error> {
    ...
    let mut found_files = HashSet::<String>::new();

    for path in block_file_paths {
        let file_contents = fs::read(path.clone()).map_err(Error::IoError)?;
        let mut tiles: HashMap<u8, HashSet<usize>> = HashMap::new();

        for (i, tile) in file_contents.iter().enumerate() {
            let block_pos = i / 16;
            let values = tiles.entry(*tile).or_default();

            values.insert(block_pos);
        }

        let filename = path.file_name().unwrap().to_str().unwrap().to_owned();
        res.push(Tileset::new(filename.clone(), tiles));
...
fn remove_uninteresting_tiles(used_tiles_per_tileset: &mut Vec<Tileset>) {
    let mut tiles_of_interest = HashSet::<u8>::new();
    tiles_of_interest.insert(0x05);
    tiles_of_interest.insert(0x23);
    tiles_of_interest.insert(0x02);
    tiles_of_interest.insert(0x04);
    tiles_of_interest.insert(0x01);

    for tileset in &mut *used_tiles_per_tileset {
        tileset.remove_uninteresting_tiles(&tiles_of_interest);
    }
...
fn check_map_block_usage(map: &Map, tilesets: &Vec<Tileset>) -> bool {
    let tileset = tilesets.iter().find(|&v| map.tileset_file() == v.filename);
    let tileset = match tileset {
        None => return false,
        Some(tileset) => tileset
    };

    for used_tile_block in tileset.used_tiles.values() {
        let intersect_count = used_tile_block.intersection(&map.used_blocks).count();
...
fn check_map_strings(maps: &Vec<Map>) -> Result<(), Error> {
    for map in maps {
        ...
        let playg_find = map_asm.find("<PLAY_G>");
        let hidden_item_find = map_asm.find("BGEVENT_ITEM");

        if player_find.is_none() && playg_find.is_none() && hidden_item_find.is_none() {
            continue;
        }

        println!("Found candidate map: {} (player_find: {:?}, playg_find: {:?}, hidden_item_find: {:?})", map.name(), player_find, playg_find, hidden_item_find);
    }
...
fn get_tilemaps_answering_constraints() -> Result<Vec<Tileset>, Error> {
    ...
    let mut res: Vec<Tileset> = Vec::new();
    let mut tiles_of_interest = HashSet::<u8>::new();
    tiles_of_interest.insert(0x05);
    tiles_of_interest.insert(0x23);
    tiles_of_interest.insert(0x01);
    ...
    for path in block_file_paths {
        let file_contents = fs::read(path.clone()).map_err(Error::IoError)?;
        let consecutive_tiles_found = file_contents.windows(2).any(|w| w[0] == 0x02 && w[1] == 0x04);

        if consecutive_tiles_found {
            let mut has_other_tiles = false;

            for tile in &tiles_of_interest {
                has_other_tiles = file_contents.iter().any(|v| v == tile);
            ...
            if has_other_tiles {
                let filename = path.file_name().unwrap().to_str().unwrap().to_owned();
                res.push(Tileset::new(filename.clone(), HashMap::new()));
            }
        }
    }
...
fn remove_maps_not_using_tileset(maps: &mut Vec<Map>, tilesets: &Vec<Tileset>) {
    maps.retain(|map| tilesets.iter().any(|tileset| tileset.filename == map.tileset_file));
}
...
fn main() -> Result<(), Error> {
    ...
    for map in &maps {
        if !found_tileset_files.contains(map.tileset_file()) {
            panic!();
        }
    }
